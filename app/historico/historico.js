...
(function(){
  ...
  const LS_ITENS='got2cook_historico_itens', LS_FILTROS='got2cook_historico_filtros', LS_MINHAS='got2cook_minhas_receitas';
  ...
  function seedIfEmpty(){
    ...
    const agora = new Date(), iso=(d)=> d.toISOString();
    lsSet(LS_ITENS, [
  ...
  function dentroDoPeriodo(iso, periodo){
    ...
    if(periodo==='hoje'){ const d=new Date(iso), now=new Date(); return d.toDateString()===now.toDateString(); }
    const dias=Number(periodo)||0, t=new Date(iso).getTime(), lim=Date.now()-(dias*24*60*60*1000);
    return t>=lim;
  }
  ...
  function labelData(d){
    const hoje=new Date(), ontem=new Date(Date.now()-24*60*60*1000);
  ...
  function agrupaPorData(arr){
    ...
    arr.forEach(it=>{
      const d=new Date(it.dataISO), lbl=labelData(d);
  ...
  const jaEstaEmMinhas=(item)=> !!getMinhas().find(r => (item.receitaId && r.receitaId===item.receitaId) || (item.id && r.originId===item.id));
  const adicionarEmMinhas=(item)=>{
    if(jaEstaEmMinhas(item)) return false;
    const arr=getMinhas();
    arr.push({ id:'m_'+(item.id||Math.random().toString(36).slice(2)), originId:item.id||null, receitaId:item.receitaId||null, titulo:item.titulo||'Receita', foto:item.foto||'', duracaoMin:item.duracaoMin||null, porcoes:item.porcoes||null, tags:Array.isArray(item.tags)?item.tags.slice(0):[], dataSalvoISO:new Date().toISOString() });
    setMinhas(arr); return true;
  };
  const removerDeMinhas=(item)=>{
    let arr=getMinhas(); const before=arr.length;
    arr=arr.filter(r=> !((item.receitaId && r.receitaId===item.receitaId) || (item.id && r.originId===item.id)));
    setMinhas(arr); return arr.length!==before;
  };
  ...
  function render(){
    ...
    if(filtrado.length===0){
      ul.innerHTML=''; vazio.hidden=false; pintarCalendarioDias(itens); return;
    }
    vazio.hidden=true;

    const grupos = agrupaPorData(filtrado);
    ul.innerHTML=''; let idxAnim=0;

    grupos.forEach((lista,tituloData)=>{
      const liData=document.createElement('li'); liData.className='li-data'; liData.textContent=tituloData; ul.appendChild(liData);

      lista.forEach(it=>{
        const salvo=jaEstaEmMinhas(it);
        const li=document.createElement('li'); li.className='item'+(salvo?' salvo':''); li.setAttribute('role','button'); li.setAttribute('tabindex','0'); li.dataset.id=it.id;

        const thumb=document.createElement('div'); thumb.className='thumb';
        const img=document.createElement('img'); img.src=(it.foto&&it.foto.trim())?it.foto:'receita_placeholder.png'; img.alt=''; thumb.appendChild(img);
        ...
        const d=new Date(it.dataISO); const hora=d.toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'});
        meta.textContent=`${d.toLocaleDateString('pt-BR')} • ${hora} • ⏱ ${it.duracaoMin||'--'} min • 🍽 ${it.porcoes||'--'} porções`;
        const tags=document.createElement('div'); tags.className='tags'; (it.tags||[]).forEach(t=>{ const s=document.createElement('span'); s.className='tag'; s.textContent=t; tags.appendChild(s); });
        bloco.append(h,meta,tags);

        const acoes=document.createElement('div'); acoes.className='acoes';
        const btnVer=document.createElement('button'); btnVer.type='button'; btnVer.className='icon-btn btn-ver'; btnVer.setAttribute('aria-label','Visualizar receita'); btnVer.title='Visualizar'; btnVer.textContent='👁️';
        const btnHeart=document.createElement('button'); btnHeart.type='button'; btnHeart.className='icon-btn btn-heart'; btnHeart.setAttribute('aria-pressed', salvo?'true':'false'); btnHeart.classList.toggle('heart-ativo', !!salvo); btnHeart.title=salvo?'Remover de Minhas Receitas':'Adicionar a Minhas Receitas'; btnHeart.textContent=salvo?'❤️':'🤍';
        const btnDel=document.createElement('button'); btnDel.type='button'; btnDel.className='icon-btn btn-trash'; btnDel.setAttribute('aria-label','Remover do histórico'); btnDel.title='Remover'; btnDel.textContent='🗑️';
        ...
        btnHeart.addEventListener('click', (ev)=>{
          ...
          if(ja){
            if(removerDeMinhas(it)){
              btnHeart.setAttribute('aria-pressed','false'); btnHeart.classList.remove('heart-ativo'); btnHeart.textContent='🤍'; btnHeart.title='Adicionar a Minhas Receitas';
              li.classList.remove('salvo');
              const all=lsGet(LS_ITENS, []); const idx=all.findIndex(x=>x.id===it.id); if(idx>-1){ all[idx].favorito=false; lsSet(LS_ITENS, all); }
            }
          }else{
            if(adicionarEmMinhas(it)){
              btnHeart.setAttribute('aria-pressed','true'); btnHeart.classList.add('heart-ativo'); btnHeart.textContent='❤️'; btnHeart.title='Remover de Minhas Receitas';
              li.classList.add('salvo');
              const all=lsGet(LS_ITENS, []); const idx=all.findIndex(x=>x.id===it.id); if(idx>-1){ all[idx].favorito=true; lsSet(LS_ITENS, all); }
            }
          }
        });

        btnDel.addEventListener('click', (ev)=>{ ev.stopPropagation(); pulse(btnDel); removerItem(it.id); });
      });
    });
  ...
  function removerItem(id){
    ...
    let itens=lsGet(LS_ITENS, []); itens=itens.filter(x=>x.id!==id); lsSet(LS_ITENS, itens);
    render();
  }

  inpBusca.addEventListener('input', ()=>{ filtros.busca=inpBusca.value||''; lsSet(LS_FILTROS, filtros); render(); });
  btnLimparBusca.addEventListener('click', ()=>{ inpBusca.value=''; filtros.busca=''; lsSet(LS_FILTROS, filtros); render(); inpBusca.focus(); });
  Array.from(chipsPeriodo).forEach(chip=>{
    chip.addEventListener('click', ()=>{
      ...
      filtros.periodo=chip.dataset.periodo; filtros.dataSelecionada=null; lsSet(LS_FILTROS, filtros); render();
    });
  });
  ...
  function gerarCalendario(mes, ano, itensParam){
    ...
    const diasComItens=new Set(); itens.forEach(it=>{ try{ diasComItens.add(toYMD(new Date(it.dataISO))); }catch{} });

    for(let i=0;i<primeiroDia;i++){ const v=document.createElement('div'); v.setAttribute('aria-hidden','true'); diasContainer.appendChild(v); }
    for(let dia=1; dia<=ultimoDia; dia++){
      const divDia=document.createElement('div'); divDia.textContent=String(dia);
      const ymd=toYMDstr(ano,mes,dia);
      if(diasComItens.has(ymd)){ divDia.classList.add('ativo'); divDia.setAttribute('title','Há receitas neste dia'); }
      ...
      divDia.addEventListener('click', ()=>{
        ...
        filtros.dataSelecionada=(filtros.dataSelecionada===ymd)?null:ymd; lsSet(LS_FILTROS, filtros); render();
  ...
  function criarFolhaClone(dir){ const c=calPage.cloneNode(true); removerIdsDoClone(c); c.classList.add('tear-page', dir==='next'?'anim-up':'anim-down'); return c; }
  function limparFolha(){ tearStage.innerHTML=''; }
  function changeMonthInstant(dir){
    if(dir==='prev'){ mesAtual--; if(mesAtual<0){ mesAtual=11; anoAtual--; } }
    else { mesAtual++; if(mesAtual>11){ mesAtual=0; anoAtual++; } }
    gerarCalendario(mesAtual, anoAtual);
  }
  function tearAndChangeMonth(dir){
    if(mesAnimando) return; mesAnimando=true;
    if(window.matchMedia('(prefers-reduced-motion: reduce)').matches){ changeMonthInstant(dir); mesAnimando=false; return; }
    const folha=criarFolhaClone(dir); tearStage.appendChild(folha); changeMonthInstant(dir);
    folha.addEventListener('animationend', ()=>{ limparFolha(); mesAnimando=false; }, {once:true});
  }
  ...
  btnLimparDia?.addEventListener('click', ()=>{ const b=btnLimparDia; b.classList.add('pulse'); b.addEventListener('animationend',()=>b.classList.remove('pulse'),{once:true}); filtros.dataSelecionada=null; lsSet(LS_FILTROS, filtros); render(); });

  function init(){ seedIfEmpty(); syncControlesFromState(); skeleton.hidden=false; gerarCalendario(mesAtual, anoAtual); setTimeout(render,150); }
  init();
})();
