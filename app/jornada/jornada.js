document.addEventListener('DOMContentLoaded', () => {
  ...
  const estilosCfg={labels:['Massas','Doces','Carnes','Saladas','Petiscos'],data:[44.4,27.8,16.7,9.1,3.0],colors:['#705a89','#b47cc5','#c5b4e3','#d3e3d3','#f2f2f2']};
  const momentosCfg={labels:['Manhã','Meio-Dia','Noite','Madrugada','Tarde'],data:[38.9,27.8,22.2,9.1,3.0],colors:['#8a9ebf','#a97dac','#c9b9d3','#e0dff2','#f2f2f2']};
  function montaChart(canvasId, legendaId, cfg){
    const canvas=$(canvasId), ul=$(legendaId); if(!canvas||!ul||!window.Chart) return;
    new Chart(canvas,{type:'pie',data:{labels:cfg.labels,datasets:[{data:cfg.data,backgroundColor:cfg.colors}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}}}});
  ...
  const state={metrics:{total:Number(localStorage.getItem('totalReceitas')||0),criadas:0,repeticoes:0,rapidas:0,improviso:0,salvas:0,cozinhas:0,saudaveis:0,noturnas:0},niveis:Object.fromEntries(MEDALHAS.map(m=>[m.id,0]))};
  if(state.metrics.criadas===0&&state.metrics.total>0){state.metrics.criadas=state.metrics.total;}
  const grid=$('conquistasContainer');
  function estrelasMarkup(n){return Array.from({length:3},(_,i)=>`<span style="color:${i<n?'#f8c100':'#999'}">★</span>`).join('');}

  function tooltipMarkup(m,nivel,valor){
    if(nivel>=3){
      return `<span class="tooltip"><span class="desc">Tudo completo ✨</span></span>`;
    }
    ...
    return `<span class="tooltip">
      ...
      <span class="next">Meta: ${target}</span>
  ...
  function renderMedalha(m){
    ...
    div.className='medalha';div.id=`c_${m.id}`;
    const primary=`../assets/${m.id}.png`;const fallback=`/assets/${m.id}.png`;
    div.innerHTML=`
  ...
  function renderTodas(){if(!grid)return;grid.innerHTML='';MEDALHAS.forEach(m=>grid.appendChild(renderMedalha(m)));}
  function calcNivel(v,t){let n=0;if(v>=t[0])n=1;if(v>=t[1])n=2;if(v>=t[2])n=3;return n;}
  function recomputeLevels(){
    MEDALHAS.forEach(m=>{
      const v=Number(state.metrics[m.metric]||0);
      const novo=calcNivel(v,m.req);
      if(novo!==state.niveis[m.id])state.niveis[m.id]=novo;
      const el=document.getElementById(`c_${m.id}`);
      if(el){
        el.querySelector('.estrelas').innerHTML=estrelasMarkup(state.niveis[m.id]);
        const tip=el.querySelector('.tooltip');
        if(tip)tip.outerHTML=tooltipMarkup(m,state.niveis[m.id],v);
      }
    });
  }
  function setMetrics(p){Object.assign(state.metrics,p||{});if(p.total!=null&&state.metrics.criadas===0&&state.metrics.total>0){state.metrics.criadas=state.metrics.total;}recomputeLevels();}
  function inc(metric,delta=1){state.metrics[metric]=Number(state.metrics[metric]||0)+delta;recomputeLevels();}
  function setNivel(id,nivel){const m=MEDALHAS.find(x=>x.id===id);if(!m)return;state.niveis[id]=Math.max(0,Math.min(3,Number(nivel)||0));const el=document.getElementById(`c_${id}`);if(el){el.querySelector('.estrelas').innerHTML=estrelasMarkup(state.niveis[id]);const v=Number(state.metrics[m.metric]||0);const tip=el.querySelector('.tooltip');if(tip)tip.outerHTML=tooltipMarkup(m,state.niveis[id],v);}}
  window.G2C=Object.assign(window.G2C||{},{setMetrics,inc,setNivel,_state:state});

  renderTodas();recomputeLevels();
  const contador=$('contador');if(contador)contador.textContent=state.metrics.total||0;
});
